...
if (missingRefs.length > 0) {
  ...
  missingRefs.forEach(ref => {
    console.error(`  - サブジャンル "${ref.subgenre}" の bookId "${ref.missingId}"`);
  });
  process.exit(1);
} else {
  ...
  // 新規追加書籍を確認
  const newBooks = ['hateshinaki-nagare-no-hate-ni', 'adobado', 'the-great-silence'];
  console.log('\n📖 新規追加書籍:');
  newBooks.forEach(id => {
    if (books[id]) {
